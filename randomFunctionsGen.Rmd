...
ggplot(bm, aes(V1,V2))+geom_point()
```
...
y.predict <- function(x, y, x.star, cov.fun) {
  K <- Sigma(x,x)
  K.star <- Sigma(x.star, x)
  K.star %*% solve(K) %*% y
}

cov.predict <- function(x, y, x.star, cov.fun) {
  K <- Sigma(x,x)
  K.star <- Sigma(x.star, x)
  K.star.star <- Sigma(x.star, x.star)
  K.star.star - K.star %*% solve(K) %*% t(K.star)
}

gg.simulations2 <- function(x, y, x.star, cov.fun, sim.number) {
  observations <- data.table(x, y)
  y.star.bar <- y.predict(x, y, x.star, cov.fun)[,1]
  predictors <- data.table(x=x.star, y=y.star.bar)
  S <- cov.predict(x, y, x.star, cov.fun)
  ms <- y.star.bar
  msims <- melted.simulations(x.star, sim.number, ms, S)
...
observations <- data.table(x=x, y=y)
y.star.bar <- y.predict(x, y, x.star, cov.fun)
  ...
  ggplot() +
      geom_line(data = msims, aes(x=x,y=value,
          ...
          geom_point(data=observations,
                     aes(x,y), size=3, colour="red")
              geom_line(data=predictors, aes(x, y), colour='blue') +
                  geom_ribbon(aes(x.star, ymax=y.star.bar+2*sqrt(d),
                                  ymin=y.star.bar-2*sqrt(d)), alpha=0.1)
